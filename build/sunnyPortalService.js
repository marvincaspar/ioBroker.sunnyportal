...
class SunnyPortalService {
    ...
    historicalProduction(dateType, year, month, day, callback) {
        ...
        let finalJar;
        const downloadResultCallback = (err, body) => {
            const response = [[]];
            const power = [];
            const times = [];
            let date;
            ...
            for (const lineItem of lineItems) {
                ...
                if (entries[0] && entries[1]) {
                    if (dateType == dateType_1.DateType.DAY) {
                    ...
                    else if (dateType == dateType_1.DateType.MONTH) {
                    ...
                    else if (dateType == dateType_1.DateType.YEAR) {
    ...
    openInverter(jar, callback) {
        ...
        request(this.url + this.OPEN_INVERTER_URL, requestOpts, (err, httpResponse, body) => {
            ...
            this.plantOID = body.match(/<input type="hidden" name="ctl00\$HiddenPlantOID" id="ctl00_HiddenPlantOID" value="(.*)" \/>/)[1];
    ...
    setFileDate(dateType, year, month, day, jar, callback) {
        const requestOpts = {
            headers: {
...
exports.SunnyPortalService = SunnyPortalService;
